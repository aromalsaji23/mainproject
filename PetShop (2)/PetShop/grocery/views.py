...
import requests
from django.shortcuts import render
from django.conf import settings  # To access settings like GEMINI_API_KEY
...
def chatbot_view(request):
    ...
    if request.method == 'POST':
        ...
        api_key = 'AIzaSyBX7FjwaAgFT4-kBWw78U8nZ_D1YkyWdWs'  # Replace with your Gemini API key
        ...
        # Check response status and parse the response
        if response.status_code == 200:
            response_data = response.json()
            if 'candidates' in response_data and len(response_data['candidates']) > 0:
                chatbot_response = response_data['candidates'][0]['output']
            else:
...
def sales_visualization(request):
    # Fetch sales data
    sales_data = Booking.objects.all().values('book_date', 'total')  # Assuming 'total' is the sales amount
    ...
    # Convert 'book_date' to a string format for better display in the template
    sales_summary['book_date'] = sales_summary['book_date'].astype(str)

    return render(request, 'sales_visualization.html', {'sales_summary': sales_summary.to_dict(orient='records')})
